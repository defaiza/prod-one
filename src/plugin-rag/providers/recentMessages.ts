    ...
    const getRecentMessageInteractions = async (
      ...
      const formattedInteractions = recentInteractionsData.map((message) => {
        ...
        if (isSelf) {
          ...
          const entity = interactionEntityMap.get(message.entityId);
          const username = entity?.metadata && typeof entity.metadata === 'object' && typeof (entity.metadata as any).username === 'string'
            ? (entity.metadata as any).username
            : undefined;
