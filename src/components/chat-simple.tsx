...
export const Chat = ({ sessionId: propSessionId }: ChatProps = {}) => {
  ...
  return (
    <div className="min-h-screen w-full flex">
      ...
      <div
        className={clsx(
          "fixed left-0 top-0 h-full w-80 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-700 z-20 transition-transform duration-300 ease-in-out",
          showSessionSwitcher ? "translate-x-0" : "-translate-x-full"
        )}
      >
      ...
      {/* Sidebar Toggle Button - Fixed on left side */}
      <button
        onClick={() => setShowSessionSwitcher(!showSessionSwitcher)}
        className={clsx(
          "fixed left-4 top-20 z-30 p-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all duration-300 ease-in-out",
          showSessionSwitcher ? "translate-x-80" : "translate-x-0"
        )}
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className={clsx(
        ...
        showSessionSwitcher ? "ml-80" : "ml-0"
      )}>
      ...
      <div className={clsx(
        "fixed bottom-0 left-0 right-0 p-4 bg-white z-10 transition-all duration-300 ease-in-out",
        showSessionSwitcher ? "ml-80" : "ml-0"
      )}>
